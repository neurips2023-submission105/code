...
cols = ['run_id', 'seed', 'context_points', 'context_transform', 'forward_points',
        'nll_test', 'acc_sel_test', 'acc_test', 'ece_test', 'ood_auroc_entropy']

_m1 = get_summary_metrics('deeplearn/fspace-inference/9xc5ptb1')[1][cols]
_m1['dataset'] = 'CIFAR-10'

_m2 = get_summary_metrics('deeplearn/fspace-inference/dblw56j7')[1][cols]
...
# metrics.to_csv('results/all_ctx.csv', index=False)

cols = ['acc', 'sel_acc', 'nll', 'ece', 'ood_auroc']

mu = metrics.groupby(['dataset', 'context_points', 'context_transform']).mean(numeric_only=True).drop(columns=['seed'])[cols]
...
sigma = metrics.groupby(['dataset', 'context_points', 'context_transform']).std(numeric_only=True).drop(columns=['seed'])[cols]
